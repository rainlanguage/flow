...
contract FlowMulticallTest is FlowTest {
     ...
     *
     * In V4 there is no encoded dispatch, so a per-flow `eval4` mock keyed on the
     * expression no longer differentiates the two flows. The multicall calls
     * `eval4` once per flow in order, so a sequential mock returns flow A's stack
     * to the first call and flow B's stack to the second.
     */
    function testFlowBasicMulticallFlows(
        ...
        (uint256[] memory stackA,) =
            mintAndBurnFlowStack(bob, 20 ether, 10 ether, 5, transfersERC20toERC20(bob, address(flow), amount, amount));
        (uint256[] memory stackB,) = mintAndBurnFlowStack(
            bob, 20 ether, 10 ether, 5, transferERC721ToERC1155(address(flow), bob, tokenId, amount, tokenId)
        );
